    ...
    <div class="container">
        ...
        <div>
            <div v-for="category in paginatedCategories" :key="category.n_idcategoria" class="category-item">
                ...
                <button @click="deleteCategory(category.n_id_categoria)" class="delete"
...
export default {
    ...
    if (!id) {
        ...
        this.showMessage("ID de categoría no válido", "error");
        return;
    }

    if (this.isDeleting) return; // Evitar múltiples eliminaciones simultáneas
    ...
    try {
        ...
        this.showMessage("Error al eliminar categoría: " + (error.response?.data?.message || error.message), "error");
    } finally {
