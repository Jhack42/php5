...
export default {
    setup() {
        ...
        const editingUsers = ref({}); // { itemId: { userId: string, timestamp: number } }
        ...
        function isRecentlyUpdated(itemId) {
            ...
            const RECENT_UPDATE_THRESHOLD = 3000; // 3 segundos
            ...
            try {
                ...
                if (editingId.value) {
                    ...
                    if (response.data.success) {
                        ...
                        // Emitir evento de actualización normal
                        socket.emit('carousel:update', response.data.data);

                        // Emitir evento para la notificación visual de edición colaborativa
                    ...
                    if (response.data.success) {
                        ...
                        // Emitir evento de creación normal
                        socket.emit('carousel:create', response.data.data);

                        // Emitir evento para la notificación visual de creación
                        socket.emit('carousel:editing:update', {
                            itemId: response.data.data.id, // ID del nuevo item creado
                            userId: socket.id,
                            timestamp: Date.now()
                        });
                    }
        ...
        onMounted(() => {
            ...
            // Nuevos eventos de edición colaborativa
            ...
            socket.on('carousel:editing:updated', (data) => {
                ...
                // Actualizar la lista después de una edición
                loadItems();
            });
        });
        ...
        return {
            ...
            showCreateModal,
            toasts,
            editingUsers, // Nuevo estado para edición colaborativa
