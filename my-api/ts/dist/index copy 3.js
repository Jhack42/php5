...
const businessLogicController_1 = require("./controllers/businessLogicController");
// Crear la aplicación Express
const app = (0, express_1.default)();
const port = 3000;
const BASE_URL = process.env.BASE_URL || "http://127.0.0.1:3000";
// Habilitar CORS y manejar JSON
app.use((0, cors_1.default)());
app.use(express_1.default.json());
// Crear una instancia del controlador de lógica de negocio
const businessController = new businessLogicController_1.BusinessLogicController();
// API de negocio: procesar datos
app.post('/api/logic/process', (req, res) => {
    const inputData = req.body;
    const result = businessController.processData(inputData);
    res.json(result);
});
// API de negocio: cálculo
app.get('/api/logic/calculate/:value', (req, res) => {
    const value = parseInt(req.params.value, 10);
    if (isNaN(value)) {
        return res.status(400).json({ error: "El valor debe ser un número" });
    }
    const result = businessController.calculate(value);
    res.json({ originalValue: value, result });
});
