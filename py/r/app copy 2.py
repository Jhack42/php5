...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(units=10, activation='relu', name='hidden_layer2'),
    tf.keras.layers.Dense(units=10, activation='relu', name='hidden_layer3'),
...
class LiveWeightsCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        if epoch % 100 == 0:  # Enviar actualización cada 10 épocas
            weights_data = []
