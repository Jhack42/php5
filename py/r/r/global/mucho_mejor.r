...
server <- function(input, output, session) {
  ...
  loss_history <- reactiveVal(NULL)  # Historia de las pérdidas
  ...
  observeEvent(input$start_training, {
    ...
    losses <- c()  # Inicializa la lista para almacenar las pérdidas
    ...
    for (epoch in 1:input$epochs) {
      ...
      # Extrae la pérdida de la última época
      history <- h2o.scoreHistory(model_h2o)
      losses <- c(losses, history$training_rmse[nrow(history)])  # Almacena la pérdida
    }
    
    # Guardar la historia de pérdidas
    loss_history(losses)
