<template>
    <div class="eventos-component columns">
        ...
        <div :class="posicionBarra.includes('lateral') ? 'column is-one-quarter' : ''">
            <aside v-if="posicionBarra.includes('lateral')"
                :class="['menu', { 'is-left': posicionBarra === 'lateral-izquierda', 'is-right': posicionBarra === 'lateral-derecha' }]">
        ...
        <div v-if="mostrarConfiguracion" class="modal is-active">
            ...
            <div class="modal-content">
                <div class="box">
                    ...
                    <!-- Nueva sección para configurar el orden de los filtros -->
                    <div v-if="configuracionActiva === 'orden-filtros'">
                        <label>Orden de los filtros:</label>
                        <div class="select">
                            <select v-model="ordenFiltros">
                                <option value="column">Vertical</option>
                                <option value="row">Horizontal</option>
                            </select>
                        </div>
                        <button @click="guardarOrdenFiltros" class="button is-success is-small">Guardar Orden</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        moverFiltro(index, direccion) {
            const newIndex = index + direccion;
            if (newIndex >= 0 && newIndex < this.opcionesFiltros.length) {
                const temp = this.opcionesFiltros[index];
                this.opcionesFiltros[index] = this.opcionesFiltros[newIndex];
                this.opcionesFiltros[newIndex] = temp;
            }
        },
...
.menu.is-left {
    order: -1;
    /* Barra de filtros a la izquierda */
}
